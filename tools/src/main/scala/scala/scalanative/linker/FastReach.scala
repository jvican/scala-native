...
class FastReach(config: build.Config, entries: Seq[Global], loader: ClassLoader) {
  ...
  type ConcurrentHashSet[T] = ConcurrentHashMap.KeySetView[T, java.lang.Boolean]
  object ConcurrentHashSet {
      def apply[T](): ConcurrentHashSet[T] = ConcurrentHashMap.newKeySet[T]
  }

  val dyncandidates = new ConcurrentHashMap[Sig, ConcurrentHashSet[Global]]
  val dynsigs2 = ConcurrentHashSet[Sig]
  val dynimpls2 = ConcurrentHashSet[Global]
      ...
      info.responds.foreach {
        case (sig: Sig.Method, impl) =>
          ...
          if (!dynsigs.contains(dynsig)) {
            lazy val empty = ConcurrentHashSet[Global]()
            dyncandidates.putIfAbsent(dynsig, empty) match {
              case null => empty.add(impl)
              case previous => previous.add(impl)
            }
          } else {
            dynimpls += impl
            reachGlobal(impl)
  ...
  def reachDynamicMethodTargets(dynsig: Sig) = {
    if (!dynsigs.contains(dynsig)) {
      ...
      var dynsigCandidates: ConcurrentHashSet[Global] = null
      // TODO: Needs to be changed
      dyncandidates.computeIfPresent(
        dynsig,
        new BiFunction[Sig, ConcurrentHashSet[Global], ConcurrentHashSet[Global]] {
          def apply(sig: Sig, candidates: ConcurrentHashSet[Global]): ConcurrentHashSet[Global] = {
            dynsigCandidates = candidates
            ConcurrentHashSet()
          }
        }
      )

      if (dynsigCandidates != null) {
        dynsigCandidates.forEach {
          new Consumer[Global] {
            def accept(impl: Global): Unit = {
              dynimpls += impl
              reachGlobal(impl)
            }
          }
